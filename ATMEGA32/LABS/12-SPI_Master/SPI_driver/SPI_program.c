...
void SPI_voidInit(void)
{
	...
	#elif	SPI_PRESCALER == DIVIDED_BY_4
			CLR_BIT(SPSR, SPSR_SPI2X);
			CLR_BIT(SPCR, SPCR_SPR1);
			CLR_BIT(SPCR, SPCR_SPR0);

	#elif	SPI_PRESCALER == DIVIDED_BY_8
			SET_BIT(SPSR, SPSR_SPI2X);
			CLR_BIT(SPCR, SPCR_SPR1);
			SET_BIT(SPCR, SPCR_SPR0);

	#elif	SPI_PRESCALER == DIVIDED_BY_16
			CLR_BIT(SPSR, SPSR_SPI2X);
			CLR_BIT(SPCR, SPCR_SPR1);
			SET_BIT(SPCR, SPCR_SPR0);

	#elif	SPI_PRESCALER == DIVIDED_BY_32
			SRT_BIT(SPSR, SPSR_SPI2X);
			SRT_BIT(SPCR, SPCR_SPR1);
			CLR_BIT(SPCR, SPCR_SPR0);

	#elif	SPI_PRESCALER == DIVIDED_BY_64
			CLR_BIT(SPSR, SPSR_SPI2X);	// or set
			SET_BIT(SPCR, SPCR_SPR1);
			CLR_BIT(SPCR, SPCR_SPR0);	// or set

	#elif	SPI_PRESCALER == DIVIDED_BY_128
			CLR_BIT(SPSR, SPSR_SPI2X);
			SET_BIT(SPCR, SPCR_SPR1);
			SET_BIT(SPCR, SPCR_SPR0);
